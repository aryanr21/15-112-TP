...
def appStarted(app):
    ...
    #Dashboard variables
    app.dashButtons = [[(app.width//3,app.height//3),'Stock ticker']]
...
def dashboardScreen_redrawAll(app,canvas):
    canvas.create_text(app.width//2,app.height//2,text=f'Hello {app.profile[0]}, which stock chart do you want to see?!', font='Times 24')
    drawButton(app,canvas,app.dashButtons)

def dashboardScreen_mousePressed(app,event):
    if checkBounds(app,event.x,event.y,app.dashButtons) != False:
        ticker = app.getUserInput('please enter stock ticker')
        app.showMessage('Thank you, the system will now print monthly values from January 2015 to January 2021.')
        stockdata = stock(ticker)
        stockdata.chartGenerator([2015,1,1],[2021,1,1],'1mo')


def dashboardScreen_keyPressed(app,event):
    if event.key == 'r':
        app.profile = list()
        app.mode = 'accountScreen'
