...
def appStarted(app):
    ...
    app.stockSearch = [[(app.width//9,app.height//3),'Stock search']]
      #Variables for graphing
    app.graphFlag = True
    app.defaultMessage = 'Click on the stock you would like to look at!'
    app.data = getChart('NDAQ',[2010,1,1],[2021,11,1],'1mo')
    ...
    #eyeTrack variables
    app.eyetrackButtons = [[(app.width//5,app.height//5),'Sentiment Analysis'],[(app.width//3,app.height//5),'Investing analysis']]
...
def dashboardScreen_mousePressed(app,event):
    ...
    if positionCheck != False:
        if positionCheck == (1069.0, 166):
            ...
            app.profile = list()
            app.mode = 'accountScreen'
    elif checkBounds(app,event.x,event.y,app.stockSearch) == (222.0,666.0):
...
def dashboardScreen_redrawAll(app,canvas):
    ...
    if app.graphFlag == True:
        canvas.create_rectangle(app.width//2 - app.width//15,app.height//2.3 - app.height//20, app.width//1.2 + app.width//8, app.height//1.2 + app.height//20, fill='light blue')
    ...
    if clickActivity != False:
        if clickActivity == (200,200):
            ...
            if (stockTicker != None) and (stockPrice != None) and (stockQuantity != None): 
                ...
                app.portfolio.append([stockTicker,int(stockPrice),int(stockQuantity),datetime.now()])
...
def portfolio(app,canvas):
    ...
    for stocks in app.portfolio:
        ...
        investmentValue = stocks[1]*stocks[2]
        presentValue = stockPrice*stocks[2]
        ...
        canvas.create_text(app.width//6,app.height//2.7 + space,text = stocks[1], fill='Black',font='Times 15 bold')
...
def eyeTrackScreen_mousePressed(app,event):
    ...
    if featureCheck != False:
        if featureCheck == (400,400):
            app.mode = 'sentimentScreen'
        elif featureCheck == (666,400):
            app.mode = 'habitAnalyzerScreen'  
...
#####################################
# Step 5a: Sentiment screen
#####################################

def sentimentScreen_mousePressed(app,event):
    positionCheck = checkBounds(app,event.x,event.y,app.navButtons)   
    if positionCheck != False:
        if positionCheck == (1069.0, 166):
            app.mode = 'portfolioScreen'
...
def trackSentiment(app,canvas):
    spacing = app.portfolioSpace
    for values in app.portfolio:
        print(values)
        buySentiment = buyingSentimentAnalyzer(trendCalculator(dataExtractor(values[0],values[3])),values[0])
        competitorCompare = similarCompanyTrend(values[0],trendCalculator(dataExtractor(values[0],values[3])),values[3])
        canvas.create_text(app.width//2,app.height//2.4 + spacing,text = buySentiment,fill='black',font='Times 14 bold')
        spacing += app.portfolioSpace
        canvas.create_text(app.width//2,app.height//2.4 + spacing,text = competitorCompare,fill='black',font='Times 14 bold')
        spacing += app.portfolioSpace

def sentimentScreen_redrawAll(app,canvas):
    canvas.create_image(app.width//2, app.height//2, image=ImageTk.PhotoImage(app.landingScreenImage))
    screenTop(app,canvas)
    drawButton(app,canvas,app.navButtons)  
    canvas.create_rectangle(app.width//6,app.height//2.4,app.width//1.2,app.height//1.1,fill = 'light blue') 
    trackSentiment(app,canvas)

#####################################
# Step 5b: Habit analyzing screen
#####################################

def habitAnalyzerScreen_mousePressed(app,event):
    positionCheck = checkBounds(app,event.x,event.y,app.navButtons)   
    if positionCheck != False:
        if positionCheck == (1069.0, 166):
            app.mode = 'portfolioScreen'
...
def writeHabit(app,canvas):
    spacing = app.portfolioSpace
    for values in app.portfolio:
        analysis = habitAnalyzer(trendCalculator(dataExtractor(values[0],values[3])),values[0])
        if analysis != None:
            canvas.create_text(app.width//2,app.height//2.4 + spacing,text = analysis,fill='black',font='Times 14 bold')
            spacing += app.portfolioSpace             

def habitAnalyzerScreen_redrawAll(app,canvas):
    canvas.create_image(app.width//2, app.height//2, image=ImageTk.PhotoImage(app.landingScreenImage))
    screenTop(app,canvas)
    drawButton(app,canvas,app.navButtons)  
    canvas.create_rectangle(app.width//6,app.height//2.4,app.width//1.2,app.height//1.1,fill = 'light blue')
    writeHabit(app,canvas)
