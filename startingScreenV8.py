...
from portfolioAnalyzerV1 import *
...
def appStarted(app):
    ...
    app.stockSearch = [[(app.width//15,app.height//3),'Stock search'],[(app.width//6.5,app.height//3),'Graph it!']]
    app.counter = -1
      #Variables for graphing
    app.graphFlag = False
    app.defaultMessage = "Click graph it button to see stock chart starting from first company! Click again to see next company."
    app.data = []
    ...
    #Portfolio variables
    app.portfolio = []
    app.stockBuyCounter = 0
    app.habitDefaultMessage = 'Not enough data to assess habits'
    app.sellFlag = False
    app.rebuyFlag = False
    app.portfolioSpace = app.height//60
    app.portfolioButtons = [[(app.width//10,app.height//10),'Buy'],[(app.width//5,app.height//10),'Sell']]

    #eyeTrack variables
    app.eyetrackButtons = [[(app.width//3.5,app.height//4.8),'Click here!']]
    app.analysisFlag = False

    #Analysis screen variables
    app.company = str()
    app.companyData = []
...
def drawYLabel(app,canvas,priceList,yUnits):
    startXVal = app.width//2
    startYVal = app.height//1.2
    intervalList = intervalCreator(max(priceList) + max(priceList)/2)
    for values in intervalList:
        canvas.create_text(startXVal - app.width//40,startYVal - values*yUnits,text=values,font = 14)
...
def drawXLabel(app,canvas,timeRange,xUnits):
    ...
    for values in timeInterval:
        canvas.create_text(startXVal + values*xUnits,startYVal + app.height//40,text=timeRange[values],font = 14)    

def chartLabel(app,canvas):
    companyTicker = app.stockNamesScreens[0][app.counter]
    canvas.create_text(app.width//1.43,app.height//2.5,text=f'{companyTicker} period 2016-2021',font='Times 17 bold')
...
def drawLine(app,canvas,priceRange,dayRange):
    ...
    startXVal = app.width//2
    startYVal = app.height//1.2
    finalXVal = app.width//1.1
    finalYValue = app.height//2.3
...
def dashboardScreen_mousePressed(app,event):
    ...
    if positionCheck != False:
        if positionCheck == (1069.0, 166):
            ...
            app.profile = list()
            app.mode = 'accountScreen'
    elif checkBounds(app,event.x,event.y,app.stockSearch) == (133,666):
        stockTicker = app.getUserInput('Please enter the stock ticker you want to add to your list!')
        if stockTicker != None:
            tempCheck = stock(stockTicker)
            if tempCheck.companyName() == "Enter a valid ticker!":
                app.showMessage('Please enter a valid ticker!')
            else:
                app.stockNamesScreens[0].append(stockTicker)
    
    elif checkBounds(app,event.x,event.y,app.stockSearch) == (307.0,666):
        if app.counter >= len(app.stockNamesScreens[0])-1:
            app.counter = -1
        app.counter += 1
        companyTicker = app.stockNamesScreens[0][app.counter]
        app.data = getChart(companyTicker,[2016,1,1],[2021,1,1],'1mo')
        app.graphFlag = True
...
def dashboardScreen_redrawAll(app,canvas):
    ...
    canvas.create_text(app.width//3.5,app.height//2 - app.height//25,text='Price',fill='Blue',font='Times 17 bold italic')
    canvas.create_text(app.width//5,app.height//2 - app.height//25,text='Change',fill='Blue',font='Times 17 bold italic')
    stockScreen(app,canvas,0)
    screenTop(app,canvas)
    drawButton(app,canvas,app.navButtons)
    drawButton(app,canvas,app.stockSearch)
    canvas.create_rectangle(app.width//2 - app.width//15,app.height//2.3 - app.height//13, app.width//1.2 + app.width//8, app.height//1.2 + app.height//20, fill='light blue')
    canvas.create_text(app.width//1.43,app.height//1.7,text=app.defaultMessage,font = 'Times 17 bold')
    if app.graphFlag == True:
        canvas.create_rectangle(app.width//2 - app.width//15,app.height//2.3 - app.height//13, app.width//1.2 + app.width//8, app.height//1.2 + app.height//20, fill='light blue')
        canvas.create_line(app.width//2,app.height//1.2,app.width//1.1,app.height//1.2,width=2)
        canvas.create_line(app.width//2,app.height//1.2,app.width//2,app.height//2.3,width=2)
...
def investmentValue(app,canvas):
    if len(app.portfolio) == 0:
        initialInvestment = 0
        finalValue = 0
        netPortfolioValue = 0
        canvas.create_rectangle(app.width//1.9,app.height//3,app.width//1.35,app.height//2.5,fill='light blue')
        canvas.create_text(app.width//1.73,app.height//2.72,text='Initital investment:',font='Times 17 bold')
        canvas.create_text(app.width//1.48,app.height//2.72,text=f'${initialInvestment}',font='Times 17 bold')

        canvas.create_rectangle(app.width//1.29,app.height//3,app.width//1.03,app.height//2.5,fill='light blue')
        canvas.create_text(app.width//1.21,app.height//2.72,text='Final Investment:',font='Times 17 bold')
        canvas.create_text(app.width//1.09,app.height//2.72,text=f'${finalValue}',font='Times 17 bold')

        canvas.create_rectangle(app.width//1.6,app.height//2.2,app.width//1.2,app.height//1.9,fill='light blue')
        canvas.create_text(app.width//1.52,app.height//2.05,text='Net Value:',font='Times 17 bold')
        canvas.create_text(app.width//1.35,app.height//2.05,text=f'${netPortfolioValue}',font='Times 17 bold')

    else:
        initialInvestmentValue = 0
        finalInvestmentValue = 0
        netValue = 0
        for stocks in app.portfolio:
            initialInvestmentValue += stocks[1]*float(stocks[2])
            stockData = stock(stocks[0])
            stockPrice = stockData.priceMetrics()['Last recorded price']
            #Calculating final investment value
            presentValue = stockPrice*float(stocks[2])
            finalInvestmentValue += presentValue
            netValue += presentValue - (stocks[1]*float(stocks[2]))
        #Drawing initial investment value
        canvas.create_rectangle(app.width//1.9,app.height//3,app.width//1.35,app.height//2.5,fill='light blue')
        canvas.create_text(app.width//1.73,app.height//2.72,text='Initital investment:',font='Times 17 bold')
        canvas.create_text(app.width//1.48,app.height//2.72,text=f'${"{:.2f}".format(initialInvestmentValue)}',font='Times 17 bold')

        #Drawing final investment value
        canvas.create_rectangle(app.width//1.29,app.height//3,app.width//1.03,app.height//2.5,fill='light blue')
        canvas.create_text(app.width//1.21,app.height//2.72,text='Final Investment:',font='Times 17 bold')
        canvas.create_text(app.width//1.09,app.height//2.72,text=f'${"{:.2f}".format(finalInvestmentValue)}',font='Times 17 bold')

        #Drawing net value
        canvas.create_rectangle(app.width//1.6,app.height//2.2,app.width//1.2,app.height//1.9,fill='light blue')
        canvas.create_text(app.width//1.52,app.height//2.05,text='Net Value:',font='Times 17 bold')
        canvas.create_text(app.width//1.35,app.height//2.05,text=f'${netValue}',font='Times 17 bold')

def portfolioScreen_mousePressed(app,event):
    clickActivity = checkBounds(app,event.x,event.y,app.portfolioButtons)
    if clickActivity != False:
        if clickActivity == (200,200):
            idx = 0
            stockTicker = app.getUserInput('Please enter stock ticker')
            for stocks in app.portfolio:
                if stocks[0] == stockTicker:
                    app.rebuyFlag = True
                    break
                else:
                    idx += 1
            stockPrice = app.getUserInput('Please enter buying price')
            stockQuantity = app.getUserInput('Please enter quantity bought')
            if app.rebuyFlag == True:
                if (stockTicker != None) and (stockPrice != None) and (stockQuantity != None):
                    app.portfolio[idx][1] = ((app.portfolio[idx][1]*float(app.portfolio[idx][2])) + (float(stockPrice)*int(stockQuantity)))/(app.portfolio[idx][2] + int(stockQuantity)) #Avg stock price calculation
                    app.portfolio[idx][2] += int(stockQuantity)
                    app.rebuyFlag = False
            else:
                if (stockTicker != None) and (stockPrice != None) and (stockQuantity != None): 
                    app.stockBuyCounter += 1
                    app.showMessage('Thank you!')
                    app.portfolio.append([stockTicker,float(stockPrice),int(stockQuantity),datetime.now()])
        elif clickActivity == (400,200):
            idx = 0
            sellCompany = app.getUserInput('Please enter the ticker of the company shares you want to sell.')
            if sellCompany != None:
                for stocks in app.portfolio:
                    if stocks[0] == sellCompany:
                        app.sellFlag = True
                        break
                    else:
                        idx += 1
            
                if app.sellFlag == True:
                    sellQuantity = app.getUserInput('What quantity would you like to sell?')
                    if sellQuantity != None:
                        if int(sellQuantity) > app.portfolio[idx][2]:
                            app.showMessage(f'You do not own these many shares of {app.portfolio[idx][0]}')
                        else:
                            app.portfolio[idx][2] -= int(sellQuantity)
                            if app.portfolio[idx][2] == 0:
                                app.portfolio.pop(idx)
                    app.sellFlag = False
                else:
                    app.showMessage(f'You do not own shares of {sellCompany}')
...
def habitAnalyzerSpace(app,canvas):
    canvas.create_rectangle(app.width//1.95,app.height//1.8,app.width//1.01,app.height//1.05,fill='light blue')
    canvas.create_text(app.width//1.36,app.height//1.7,text='Investor habits',font='Times 17 bold')
    if app.stockBuyCounter < 4:
        canvas.create_text(app.width//1.34,app.height//1.35,text=app.habitDefaultMessage,font='Times 17 bold italic')
    else:
        result = concentration(companyTrendData(app.portfolio))
        spacing = app.portfolioSpace
        for trends in result:
            canvas.create_text(app.width//1.33,app.height//1.7 + spacing,text=trends,font='Times 12')
            spacing += app.portfolioSpace
            
def portfolio(app,canvas):
    space = app.portfolioSpace 
    for stocks in app.portfolio:

        #Obtaining stock price (LTP)
        stockData = stock(stocks[0])
        stockPrice = stockData.priceMetrics()['Last recorded price']

        #Calculating investment profit/loss
        investmentValue = stocks[1]*float(stocks[2])
        presentValue = stockPrice*float(stocks[2])
        #Below 2 d.p fomatting technique taken from:
        #https://pythonguides.com/python-print-2-decimal-places/
        netValue = "{:.2f}".format(presentValue - investmentValue)

        #Writing down portfolio data
        canvas.create_text(app.width//12,app.height//2.7 + space,text = stocks[0], fill='Black',font='Times 15 bold')
        canvas.create_text(app.width//6,app.height//2.7 + space,text = "{:.2f}".format(stocks[1]), fill='Black',font='Times 15 bold')
        canvas.create_text(app.width//4,app.height//2.7 + space,text = stocks[2], fill='Black',font='Times 15 bold')
        canvas.create_text(app.width//3,app.height//2.7 + space,text = stockPrice, fill='Black',font='Times 15 bold')
        canvas.create_text(app.width//2.3,app.height//2.7 + space,text = netValue, fill=valueColor(float(netValue)),font='Times 15 bold')
        space += app.portfolioSpace

def portfolioScreen_redrawAll(app,canvas):
    canvas.create_image(app.width//2, app.height//2, image=ImageTk.PhotoImage(app.landingScreenImage))
    #Lines 644-650 for portfolio area
    ...
    screenTop(app,canvas)
    drawButton(app,canvas,app.navButtons)
    drawButton(app,canvas,app.portfolioButtons)
    portfolio(app,canvas)
    investmentValue(app,canvas)
    habitAnalyzerSpace(app,canvas)


#####################################
# Step 5: eyeTrack screen
#####################################

def eyeTrackButton(app,canvas,lst):
    for values in lst:
        idx = int()
        if values[0] == (values[0][0],values[0][1]):
            idx = lst.index(values)
        canvas.create_rectangle(values[0][0],values[0][1],values[0][0]+app.buttonWidth//0.8,values[0][1]+app.buttonHeight,fill='light blue',width=2)
        canvas.create_text((2*values[0][0]+app.buttonWidth//0.8)//2,(2*values[0][1]+app.buttonHeight)//2,text = values[1],font=f"Times {app.width//80} bold")

def eyeTrackScreen_mousePressed(app,event):
    positionCheck = checkBounds(app,event.x,event.y,app.navButtons)   
    if positionCheck != False:
        if positionCheck == (1069.0, 166):
            app.mode = 'portfolioScreen'
    ...
    featureCheck = checkBounds(app,event.x,event.y,app.eyetrackButtons) 
    if featureCheck != False:
        if featureCheck == (571.0,416.0):
            companyTicker = app.getUserInput('Please enter company ticker')
            if companyTicker != None:
                app.company = companyTicker
                app.companyData = getChart(app.company,[2020,1,1],[2021,10,1],'1d')
                app.mode = 'analysisScreen'
                

def eyeTrackScreen_redrawAll(app,canvas):
    canvas.create_image(app.width//2, app.height//2, image=ImageTk.PhotoImage(app.landingScreenImage))
    canvas.create_rectangle(app.width//4,app.height//2.4,app.width//1.5,app.height//2,fill='light blue')
    canvas.create_text(app.width//2.19,app.height//2.20,text='Please enter stock ticker you want to analyze!',font='Times 18 bold')
    screenTop(app,canvas)
    drawButton(app,canvas,app.navButtons)   
    eyeTrackButton(app,canvas,app.eyetrackButtons)


# #####################################
# # Step 5a: Stock analysis screen
# #####################################
...
def analysisDrawYLabel(app,canvas,priceList,yUnits):
    startXVal = app.width//2
    startYVal = app.height//1.2
    intervalList = intervalCreator(max(priceList) + max(priceList)/2)
    for values in intervalList:
...
def analysisDrawXLabel(app,canvas,timeRange,xUnits):
    startXVal = app.width//2
    startYVal = app.height//1.2
    timeInterval = intervalCreator(len(timeRange))
    for values in timeInterval:
...
def stockChartLabel(app,canvas):
    canvas.create_text(app.width//1.43,app.height//2.5,text=f"{app.company} stock chart for period 2020-2021",font='Times 17 bold')

def analysisDrawLine(app,canvas,priceRange,dayRange):
    pointList = []
    priceList = [0]
    xPoslist = []
    startXVal = app.width//2
    ...
    analysisDrawYLabel(app,canvas,priceList,yUnits)
    analysisDrawXLabel(app,canvas,dayRange,xUnits)
    stockChartLabel(app,canvas)

def analysisScreen_mousePressed(app,event):
    positionCheck = checkBounds(app,event.x,event.y,app.navButtons)   
    if positionCheck != False:
        if positionCheck == (1069.0, 166):
            app.mode = 'portfolioScreen'
...
def stockAnalysis(app,canvas):
    analysisList = companyAnalysis(app.company)
    spacing = app.portfolioSpace
    for elements in analysisList:
        canvas.create_text(app.width//5,app.height//2.75 + spacing,text=F'-> {elements}',font='Times 14 bold')
        spacing += app.portfolioSpace
        

def analysisScreen_redrawAll(app,canvas):
    canvas.create_image(app.width//2, app.height//2, image=ImageTk.PhotoImage(app.landingScreenImage))
    screenTop(app,canvas)
    drawButton(app,canvas,app.navButtons)  
    canvas.create_rectangle(app.width//50,app.height//2.75,app.width//2.5,app.height//1.1,fill='light blue')
    canvas.create_rectangle(app.width//2 - app.width//15,app.height//2.3 - app.height//13, app.width//1.2 + app.width//8, app.height//1.2 + app.height//20, fill='light blue')
    canvas.create_line(app.width//2,app.height//1.2,app.width//1.1,app.height//1.2,width=2)
    canvas.create_line(app.width//2,app.height//1.2,app.width//2,app.height//2.3,width=2)
    analysisDrawLine(app,canvas,app.companyData.Close,app.companyData.Date)
    stockAnalysis(app,canvas)
